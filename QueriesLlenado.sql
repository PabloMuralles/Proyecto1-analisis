...
--Dimensiones
	...
	SELECT V.VehiculoID,
			...
			V.SubModelo,
			V.Estilo,
			V.FechaCreacion as FechaCreacionVehiculo
			--Columnas Auditoria
			,GETDATE() AS FechaCreacion
			,CAST(SUSER_NAME() AS nvarchar(100)) AS UsuarioCreacion
			,GETDATE() AS FechaModificacion
	...
	SELECT c.IDCotizacion
			...
			,c.FechaModificacion as FechaModificacionCotizacion
			,c.ProcesadoPor
			,c.IDAseguradora
			...
			,c.MarcadoEntrega
			,c.IDPartner
			,c.CodigoPostal as CodigoPostalCotizacion
			...
			,pr.Ciudad
			,pr.Estado
			,pr.CodigoPostal as CodigoPostalPlanta
			,pr.Pais
			,pr.TelefonoAlmacen
			,pr.FaxAlmacen
			,pr.CorreoContacto
